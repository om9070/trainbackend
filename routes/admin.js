...
router.get("/", (req, res) => {
  res.send("train seat avaliable database");
});
...
router.get("/getTrainAllSeats", async (req, res) => {
  try {
    ...
    if(tableAllSeats){
      res.status(200).send({ message: "table created successfully", tableData: tableAllSeats,status:1 })
    }else{
      res.status(200).send({ message: "table not found!", tableData: tableAllSeats,status:0 })
    }
  } catch (e) {
